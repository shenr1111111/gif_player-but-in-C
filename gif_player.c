...
static void parse_toml(Config *cfg, const char *contents)
{
    ...
    while (line && *line) {
        ...
        *eq = '\0';
        ...
        } else if (strcmp(key, "default_folder") == 0) {
            strncpy(cfg->default_folder, val, PATH_MAX - 1);
            cfg->default_folder[PATH_MAX - 1] = '\0';
...
static void load_or_create_config(Config *cfg)
{
    ...
    if (f) {
        ...
        fprintf(stderr, "Set 'video' in that file or pass a path as argument.\n");
    } else {
...
static char *expand_tilde(const char *path, char *out, size_t outsz)
{
    if (path[0] == '~' && (path[1] == '/' || path[1] == '\0')) {
        const char *home = getenv("HOME");
        if (!home || !*home) home = ".";
        int n = snprintf(out, outsz, "%s%s", home, path + 1);
        if (n < 0 || (size_t)n >= outsz) return NULL;
    } else {
        if (strlen(path) >= outsz) return NULL;
        strcpy(out, path);
    }
    return out;
}

static void resolve_video_path(const char *arg, const char *default_folder, char *out)
{
    if (strchr(arg, '/')) {
        strncpy(out, arg, PATH_MAX - 1);
        out[PATH_MAX - 1] = '\0';
        return;
    }

    if (default_folder && default_folder[0]) {
        char folder_exp[PATH_MAX];
        if (!expand_tilde(default_folder, folder_exp, sizeof(folder_exp))) {
            strncpy(out, arg, PATH_MAX - 1);
            out[PATH_MAX - 1] = '\0';
            return;
        }
        size_t flen = strlen(folder_exp);
        if (flen > 0 && folder_exp[flen - 1] == '/')
            folder_exp[flen - 1] = '\0';
        snprintf(out, PATH_MAX, "%s/%s", folder_exp, arg);
    } else {
        strncpy(out, arg, PATH_MAX - 1);
        out[PATH_MAX - 1] = '\0';
    }
...
{
    char **newargs = calloc(7, sizeof(char *));
...
int main(int argc, char *argv[])
{
    ...
    if (!background) {
        ...
        const char *video_raw = first_positional(argc, argv);
        if (!video_raw && cfg.video[0]) video_raw = cfg.video;
        if (!video_raw) {
            char cpath[PATH_MAX];
            config_path(cpath, sizeof(cpath));
            fprintf(stderr, "No video specified. Either:\n");
            fprintf(stderr, "  1. Set 'video' in %s\n", cpath);
            fprintf(stderr, "  2. Pass a path as argument: gif_player /path/to/video.gif\n");
            fprintf(stderr, "  3. Set 'default_folder' in the config and pass just the filename.\n");
            return 1;
        }

        static char video_resolved[PATH_MAX];
        resolve_video_path(video_raw, cfg.default_folder, video_resolved);
        const char *video = video_resolved;
    ...
    while (running) {
        ...
        while (SDL_PollEvent(&ev)) {
            ...
            case SDL_MOUSEMOTION:
                if (dragging) {
                    int gx, gy;
                    SDL_GetGlobalMouseState(&gx, &gy);
                    SDL_SetWindowPosition(window, gx - drag_off_x, gy - drag_off_y);
                }
                break;
            default:
                break;
        ...
        if (now - last_frame >= frame_ns) {
            ...
            SDL_UpdateTexture(texture, NULL, frames[frame_index].pixels, width * 4);
        }
